...
void Display::initialize(int argc, char** argv) {
	...
	<< "Controls:" << endl
	<< " p" << "\t\t" << "Toggle pause" << endl
	<< " i" << "\t\t" << "Toggle interaction" << endl
	<< " 0..n" << "\t\t" << "Select actuator by index" << endl
	<< " ./," << "\t\t" << "Select next/previous actuator" << endl
	<< " +/-" << "\t\t" << "Increase/decrease air pressure for selected actuator" << endl
	<< " r" << "\t\t" << "Reset air pressure for all actuators" << endl
	<< " Mouse" << "\t\t" << "Rotate view (mouse button down)" << endl
	<< " UP/DN" << "\t\t" << "Zoom in/out" << endl << endl;
...
void Display::display() {
	...
	gluLookAt(
		camera_x + ROTATION_CENTER_X,
		camera_y + ROTATION_CENTER_Y,
		camera_z + ROTATION_CENTER_Z,
	...
	if (Simulation::hasPlane()) {
		...
		glBegin(GL_TRIANGLE_FAN);
			...
			for (double j=PLANE_DIVISIONS; j>=0; j--) {
				glVertex3d(
					cos(2*M_PI*j/PLANE_DIVISIONS) * CAGE_RADIUS,
					0,
...
void Display::special(int key, int x, int y) {
	...
	switch(key) {
		case GLUT_KEY_UP: r -= ZOOM_SPEED; break;
		case GLUT_KEY_DOWN: r+= ZOOM_SPEED; break;
	}
...
void Display::keyboard(unsigned char key, int x, int y) {
	...
	switch(key) {
		...
		case 'i':
			...
			break;
		case 'T':;
		case 't': global_alpha = ( global_alpha == 1.0 ? 0.5 : 1.0 ); break;
		case '+':
			if (interactive) {
				previous = Actuator::getSelectedActuator()->getAirPressure();
				Actuator::getSelectedActuator()->setAirPressure(fmax(0,previous+PRESSURE_STEP));
				cout
					<< "Air pressure of actuator #"
					<< Actuator::getSelectedActuator()->getIndex() << " set to: "
					<< Actuator::getSelectedActuator()->getAirPressure() << endl;
			} break;
		case '-':
			if (interactive) {
				previous = Actuator::getSelectedActuator()->getAirPressure();
		...
		case 'r':
			if (interactive) {
				Actuator::resetAll(); cout << "Reset air pressure." << endl;
			} break;
		case 'K':;
		case 'k':
			cout
		...
		case 'l':
			if (interactive) {
				cout
				<< "Length of actuator #"
				<< Actuator::getSelectedActuator()->getIndex() << " is: "
				<< Actuator::getSelectedActuator()->getLength() << endl;
			}
			break;
		case 'S':;
		case 's':
		...
		case '.':
			if (interactive) {
				...
				if (ok) cout << "Selected actuator #" << Actuator::getSelectedActuator()->getIndex() << endl;
			}
			break;
		case ',':
			if (interactive) {
