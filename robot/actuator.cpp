...
Actuator::Actuator(
	...
	this->minimum_length = minimum_length;    // will be overwritten for non-linear actuators
	...
	for (int i=0; i<number_of_links; i++) {
		...
		deCasteljau(start_at, start_control, end_control, end_at, (double)i/number_of_links, &current);
		deCasteljau(start_at, start_control, end_control, end_at, (double)(i+1)/number_of_links, &next);
	...
	for (int i=0; i<number_of_links; i++) {
		...
		dJointSetSliderAxis(slider_joints[i], direction[0], direction[1], direction[2]);
		dJointSetSliderParam(slider_joints[i], dParamLoStop, distance - expansion);
		dJointSetSliderParam(slider_joints[i], dParamHiStop, distance - 2*LINK_RADIUS);
	...
	for (int i=0; i<number_of_links-1; i++) {
		...
		dJointSetHingeAxis(hinge_joints[i], hinge_axis[0], hinge_axis[1], hinge_axis[2]);
	...
	if (Actuator::verbose) {
		cout
		<< ( linear ? "Linear actuator" : "Actuator" )
		<< " created: "
		<< "min. = " << this->minimum_length
		<< " (" << minimum_length << " intended), "
		<< number_of_links << " link" << ( number_of_links != 1 ? "s" : "" ) << ", "
		<< "max. = " << this->maximum_length << ", "
		<< "mass = " << 2 * number_of_links * LINK_MASS
		<< endl;
	}
...
void Actuator::deleteAll() {
	...
	for (vector<Actuator*>::iterator ii = all_actuators.begin(); ii != all_actuators.end(); ++ii) {
		delete *ii;
	}
...
void Actuator::setAirPressure(double new_pressure) {
	...
	air_pressure = new_pressure > 0 ? ( new_pressure <= MAX_AIR_PRESSURE ? new_pressure : MAX_AIR_PRESSURE ) : 0;
	
}

double Actuator::getAirPressure() {
...
int Actuator::getNumberOfActuators() {
	
	return all_actuators.size();
...
double Actuator::getLength() {
	...
	for (int i=0; i<number_of_links; i++) {
		...
		const double distance = sqrt(
			(next_position[0]-current_position[0])*(next_position[0]-current_position[0]) +
			(next_position[1]-current_position[1])*(next_position[1]-current_position[1]) +
			(next_position[2]-current_position[2])*(next_position[2]-current_position[2])
		);
		sum += distance;
	}
...
void Actuator::connectEndTo(Primitive* primitive) {
	...
	dJointID joint = dJointCreateBall(world, 0);
	dJointAttach(joint, bodies[2*number_of_links-1], primitive ? primitive->getBody() : 0);
...
void Actuator::setVerbose(bool verbose) {
	
	Actuator::verbose = verbose;
...
inline void Actuator::deCasteljau(dVector3 p0, dVector3 p1, dVector3 p2, dVector3 p3, double t, dVector3* p) {
...
inline void Actuator::drawCylinder(int i, double adjust_beginning, double adjust_end) {
	...
	glPushMatrix();
		...
		glBegin(GL_QUAD_STRIP); // cylinder
			for (double j=ACTUATOR_DETAIL; j>=0; j--) {
				glNormal3d(cos(2*M_PI*j/ACTUATOR_DETAIL), sin(2*M_PI*j/ACTUATOR_DETAIL), 0);
				glVertex3d(
					cos(2*M_PI*j/ACTUATOR_DETAIL) * LINK_RADIUS,
...
inline void Actuator::drawCap() {
	...
	glBegin(GL_QUAD_STRIP); // cone
		for (double j=ACTUATOR_DETAIL; j>=0; j--) {
			glNormal3d(cos(2*M_PI*j/ACTUATOR_DETAIL), sin(2*M_PI*j/ACTUATOR_DETAIL), 0);
			glVertex3d(
				cos(2*M_PI*j/ACTUATOR_DETAIL) * LINK_RADIUS * 0.3,
				sin(2*M_PI*j/ACTUATOR_DETAIL) * LINK_RADIUS * 0.3,
				-LINK_RADIUS / 4
			);
			glVertex3d(
				cos(2*M_PI*j/ACTUATOR_DETAIL) * LINK_RADIUS * 1.2,
...
void Actuator::simulate() {
	...
	double length_ratio = (getLength()-minimum_length)/(maximum_length-minimum_length);
	
	for (int i=0; i<number_of_links-1; i++) {
		
		dJointSetHingeParam(hinge_joints[i], dParamLoStop, -HINGE_STOP * length_ratio);
		dJointSetHingeParam(hinge_joints[i], dParamHiStop, HINGE_STOP * length_ratio);
