...
void Primitive::deleteAll() {
	...
	for (vector<Primitive*>::iterator ii = all_primitives.begin(); ii != all_primitives.end(); ++ii) {
		delete *ii;
	}
	
}

dBodyID Primitive::getBody() {
	
	return hasParent() ? parent->getBody() : body;
	
}

dGeomID Primitive::getGeom() {
...
void Primitive::attachHinge(Primitive* primitive, dVector3 anchor, dVector3 axis) {
...
void Primitive::attachUniversal(Primitive* primitive, dVector3 anchor, dVector3 axis_1, dVector3 axis_2) {
...
void Primitive::attachUniversal(
	Primitive* primitive,
	dVector3 anchor,
	dVector3 axis_1,
	dVector3 axis_2,
...
void Primitive::setVerbose(bool verbose) {
	
	Primitive::verbose = verbose;
...
void Primitive::setParent(Primitive* parent) {
	
	this->parent = parent;
	
}

void Primitive::setGeom(dGeomID geom) {
...
void Primitive::compositeWithPrimitives(vector<Primitive*> children) {
	...
	for (vector<Primitive*>::iterator ii = children.begin(); ii != children.end(); ++ii) {
		dMass child_mass;
	...
	const dReal this_position[3] = {this_position_ptr[0], this_position_ptr[1], this_position_ptr[2]};
	...
	for (vector<pair<Primitive*, dMass> >::iterator ii = children_and_masses.begin(); ii != children_and_masses.end(); ++ii) {
	...
	const dReal center_of_mass[3] = {
		position_x_sum / mass_sum,
	...
	dQuaternion body_inverse = {body_quaternion[0], -body_quaternion[1], -body_quaternion[2], -body_quaternion[3]};
	...
	dGeomSetPosition(
		geom,
		body_inverse_matrix[0] * this_offset_x + body_inverse_matrix[1] * this_offset_y + body_inverse_matrix[2] * this_offset_z,
		body_inverse_matrix[4] * this_offset_x + body_inverse_matrix[5] * this_offset_y + body_inverse_matrix[6] * this_offset_z,
		body_inverse_matrix[8] * this_offset_x + body_inverse_matrix[9] * this_offset_y + body_inverse_matrix[10] * this_offset_z
	);
	...
	for (vector<pair<Primitive*, dMass> >::iterator ii = children_and_masses.begin(); ii != children_and_masses.end(); ++ii) {
		...
		dGeomSetPosition(
			child->getGeom(),
			body_inverse_matrix[0] * child_offset_x + body_inverse_matrix[1] * child_offset_y + body_inverse_matrix[2] * child_offset_z,
			body_inverse_matrix[4] * child_offset_x + body_inverse_matrix[5] * child_offset_y + body_inverse_matrix[6] * child_offset_z,
			body_inverse_matrix[8] * child_offset_x + body_inverse_matrix[9] * child_offset_y + body_inverse_matrix[10] * child_offset_z
		);
	...
	glPushMatrix();
		...
		glBegin(GL_QUAD_STRIP);
			for (double j=0; j<=PRIMITIVE_DETAIL; j++) {
				glNormal3d(cos(2*M_PI*j/PRIMITIVE_DETAIL), sin(2*M_PI*j/PRIMITIVE_DETAIL), 0);
				glVertex3d(
					cos(2*M_PI*j/PRIMITIVE_DETAIL) * radius,
					sin(2*M_PI*j/PRIMITIVE_DETAIL) * radius,
					length/2
				);
				glVertex3d(
					cos(2*M_PI*j/PRIMITIVE_DETAIL) * radius,
