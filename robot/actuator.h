...
#define FACTOR_A \
  0.00007687724845623249  // coefficients for function fit of actuator behavior
#define FACTOR_B 9.203893778016578
#define FACTOR_C 9.799574209874333

class Actuator {
 public:
  Actuator(dWorldID world, dSpaceID outer_space, dVector3 start_at,
  ...
  static bool setAirPressure(int index, double new_pressure);  // set air
                                                               // pressure for
                                                               // actuator
                                                               // #index (starts
                                                               // with 0)
  static double getAirPressure(
      int index);  // get air pressure for actuator #index (starts with 0)
  ...
  // one of the actuators is selected and can be treated differently (used in
  // interactive mode):

  static bool selectActuator(int index);
  static Actuator* getSelectedActuator();

// connect*To() will accept dBodyID instead of Primitive* (if class is not
// present):
  ...
  static void setVerbose(
      bool verbose);  // whether to print information on startup
 ...
 private:
  ...
  inline static void deCasteljau(dVector3 p0, dVector3 p1, dVector3 p2,
...
class Muscle : public Actuator {
 public:
  Muscle(dWorldID world, dSpaceID outer_space, dVector3 start_at,
         dVector3 start_control,  // control points for Bezier curve
         dVector3 end_control, dVector3 end_at,
         double minimum_length,  // make sure that the distance from start to
                                 // end is
...
class LinearActuator : public Actuator {
 public:
  LinearActuator(
      dWorldID world, dSpaceID outer_space, dVector3 start_at, dVector3 end_at,
      double
          minimum_length,    // make sure that the distance from start to end is
      double maximum_length  // between minimum_length and maximum_length
  );
