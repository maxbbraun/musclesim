...
class Primitive {
 public:
  ...
  void attachHinge(Primitive* primitive, dVector3 anchor, dVector3 axis,
                   ...
                   double high_stop);
  void attachUniversal(Primitive* primitive, dVector3 anchor, dVector3 axis_1,
                       dVector3 axis_2);
  void attachUniversal(Primitive* primitive, dVector3 anchor, dVector3 axis_1,
                       dVector3 axis_2,
                       double low_stop_1,  // angles in degrees
                       double high_stop_1, double low_stop_2,
                       double high_stop_2);

  // be sure to call composite[With]Primitive[s]() before attach*() or
  // connect*To()

  void compositeWithPrimitive(Primitive* child);  // combines two primitives to
                                                  // one composite (call only
                                                  // once)
  static void compositePrimitives(vector<Primitive*> primitives);  // combines
                                                                   // multiple
                                                                   // primitives
                                                                   // to one
                                                                   // composite
...
class CappedCylinder : public Primitive {
 public:
  CappedCylinder(dWorldID world, dSpaceID space, dVector3 position, dReal mass,
                 dReal density, dReal radius, dReal length);
...
class Sphere : public Primitive {
 public:
  Sphere(dWorldID world, dSpaceID space, dVector3 position, dReal mass,
...
class Box : public Primitive {
 public:
  Box(dWorldID world, dSpaceID space, dVector3 position, dReal mass,
