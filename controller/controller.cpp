...
void Controller::deactivate() { delete active_controller; }
...
ASMController::ASMController() : Controller() {
  ...
  log_file
      << "#\ttime [s]\theight of hand [mm]\theight of target [mm]\tactivity";
  for (int i = 0; i < NUMBER_OF_ACTUATORS; i++) log_file << "\tx_" << i;
  log_file << endl;
}

ASMController::~ASMController() { delete yuragi; }
...
double* ASMController::output(double* state) {
  ...
  if (fmod(Simulation::getTime(), ACTUATOR_LAG) == 0) {  // wait for actuators
    ...
    a_2 = a_2 + 10 * exp(-(Target::getY() - height) *
                         (Target::getY() - height) / 100);
    activity = a_2 * a_2;
  ...
  log_file << "\t" << Simulation::getTime() << "\t" << height << "\t"
           << Target::getY() << "\t" << activity;
...
AASMController::AASMController() : Controller() {
  ...
  log_file << "#\ttime [s]\tx [mm]\ty [mm]\tz [mm]\tdistance to target ("
...
double* AASMController::output(double* state) {
  ...
  double distance =
      sqrt((state[0] - Target::getX()) * (state[0] - Target::getX()) +
           (state[1] - Target::getY()) * (state[1] - Target::getY()) +
           (state[2] - Target::getZ()) * (state[2] - Target::getZ()));

  log_file << "\t" << Simulation::getTime() << "\t" << state[0] << "\t"
...
double Target::getX() { return Target::x; }

double Target::getY() { return Target::y; }

double Target::getZ() { return Target::z; }
